...
# --- boundaries the off-the-shelf mutation tools (StrykerJS, cosmic-ray) showed no case was standing on
CATALOG += [
    _refusal(
        "ts",
        "slot: the highest high surrogate still opens a pair",
        "ts/slot.ts",
        "    if (unit >= 0xd800 && unit <= 0xdbff) {",
        "    if (unit >= 0xd800 && unit < 0xdbff) {",
        "text_highest_scalar_value_is_one_character",
        "validate",
    ),
    _refusal(
        "ts",
        "slot: a high surrogate pairs only with a low one",
        "ts/slot.ts",
        "      if (next >= 0xdc00 && next <= 0xdfff) {",
        "      if (next <= 0xdfff) {",
        "text_high_surrogate_before_a_letter_is_invalid",
        "validate",
    ),
    _refusal(
        "py",
        "slot: the last low surrogate is a surrogate",
        "py/haltrule/slot.py",
        "    return any(0xD800 <= ord(character) <= 0xDFFF for character in text)",
        "    return any(0xD800 <= ord(character) < 0xDFFF for character in text)",
        "text_last_low_surrogate_alone_is_invalid",
        "validate",
    ),
    _refusal(
        "py",
        "slot: the characters beside the surrogate range are characters",
        "py/haltrule/slot.py",
        "    return any(0xD800 <= ord(character) <= 0xDFFF for character in text)",
        "    return any(0xD7FF <= ord(character) <= 0xE000 for character in text)",
        "text_neighbours_of_the_surrogate_range_are_characters",
        "validate",
    ),
    _refusal(
        "ts",
        "canonicalize: the last low surrogate is a surrogate",
        "ts/checkpoint.ts",
        "    if (unit >= 0xd800 && unit <= 0xdfff) {",
        "    if (unit >= 0xd800 && unit < 0xdfff) {",
        "last_low_surrogate_alone_is_unpaired",
        "canonicalize",
    ),
    _refusal(
        "ts",
        "canonicalize: only a high surrogate opens a pair",
        "ts/checkpoint.ts",
        "    if (unit >= 0xd800 && unit <= 0xdbff) {",
        "    if (unit >= 0xd800) {",
        "two_low_surrogates_are_not_a_pair",
        "canonicalize",
    ),
    _refusal(
        "py",
    ...
    _refusal(
        "py",
        "checkpoint: a negative recorded number is present",
        "py/haltrule/checkpoint.py",
        "        return value == 0 or value != value",
        "        return value <= 0 or value != value",
        "status_negative_number_is_present_not_reusable",
        "checkpoint",
    ),
    _refusal(
        "py",
        "checkpoint: a recorded NaN is absent",
        "py/haltrule/checkpoint.py",
        "        return value == 0 or value != value",
        "        return value == 0",
        "status_nan_is_missing",
        "checkpoint",
        "raised",
    ),
    _refusal(
        "py",
        "checkpoint: a recorded true is present",
        "py/haltrule/checkpoint.py",
        "    if isinstance(value, (int, float)):  # a bool is an int: False is 0\n",
        "    if isinstance(value, bool):\n        return True\n    if isinstance(value, (int, float)):  # a bool is an int: False is 0\n",
        "status_true_is_present_not_reusable",
        "checkpoint",
    ),
    _refusal(
        "ts",
        "backoff: a delay of exactly one millisecond is a delay",
        "ts/breaker.ts",
        "Number.isFinite(bounded) && bounded > 0 ?",
        "Number.isFinite(bounded) && bounded > 1 ?",
        "backoff_of_exactly_one_millisecond",
        "backoff",
    ),
    _refusal(
        "py",
        ...
        "    if math.isfinite(bounded) and bounded > 1:",
        "backoff_of_exactly_one_millisecond",
        "backoff",
    ),
]
